...
fun MainScreen() {
    ...
    ModalNavigationDrawer(
        ...
        drawerContent = {
            ...
            ModalDrawerSheet {
                Text("Menú Garage 360", modifier = Modifier.padding(16.dp), style = MaterialTheme.typography.headlineSmall)
                Divider()
                NavigationDrawerItem(
                    label = { Text(text = "Órdenes de Trabajo") },
                    selected = currentRoute == "workOrderList",
                    onClick = {
                        navController.navigate("workOrderList"){ popUpTo(navController.graph.startDestinationId); launchSingleTop = true }
                        scope.launch { drawerState.close() } // Cierra el drawer
                    },
                    icon = { Icon(Icons.Default.DirectionsCar, contentDescription = "Órdenes") }
                )
                NavigationDrawerItem(
                    label = { Text(text = "Clientes") },
                    selected = currentRoute == "clientList",
                    onClick = {
                        navController.navigate("clientList"){ popUpTo(navController.graph.startDestinationId); launchSingleTop = true }
                        scope.launch { drawerState.close() } // Cierra el drawer
                    },
                ...
                NavigationDrawerItem(
                    ...
                    onClick = {
                        navController.navigate("facturas"){ popUpTo(navController.graph.startDestinationId); launchSingleTop = true }
                        scope.launch { drawerState.close() } // Cierra el drawer
                    },
                    icon = { Icon(Icons.Default.RequestQuote, contentDescription = "Facturas") }
                )
